...
class UnreservedAvailableQuant(models.TransientModel):
    ...
    quantity = fields.Float(
        "Available quantity", digits=dp.get_precision("Product Unit of Measure")
    )
...
class MoveChangeReserveLine(models.TransientModel):
    ...
    owner_ids = fields.Many2many(
        "res.partner", string="Owner", help="Owner of the quants"
    )
    move_str = fields.Char()
    ...
    new_reserved_availability = fields.Float(
        "New Quantity To Reseve",
        digits=dp.get_precision("Product Unit of Measure"),
        readonly=True,
        help="Quantity that will be reserved for this move",
    )
    ...
    def get_picking_domain(self, move_id, quant):
        domain = [
            ("default_location_src_id", "child_of", move_id.location_id.id),
    ...
    def update_reserved_quantity(self, move, quant, new_reserved_availability):
        ...
        if (
            float_compare(
                new_reserved_availability, 0.0, precision_digits=precision_digits
            )
            > 0
        ):
    ...
    def do_reserve(self):
        ...
        if move_id.state == "assigned":
            raise ValidationError(
                _("Move is asigned, please unreserve to change asigned stock")
            )

        quants = self.wzd_id.quant_ids.filtered(
            lambda x: x.available_quantity > 0
        ...
        if move_id.location_id != quant.location_id:
            new_picking_type_id_ids = self.env["stock.picking.type"].search(
                self.get_picking_domain(move_id, quant), limit=1
            )
            ...
            if move_id.picking_type_id != new_picking_type_id:
                ...
                new_vals = self.get_move_values(
                    move_id=move_id, quant_id=quant, picking_type_id=new_picking_type_id
                )
                if new_vals:
    ...
    def button_re_reserve_move(self):
        ...
        to_reserve = self.filtered(
            lambda x: x.state in ("confirmed", "partially_available")
        ).sorted(key=lambda l: l.sequence)
...
class MoveChangeQuantWzd(models.TransientModel):
    ...
    move_line_ids = fields.One2many(
        related="move_id.move_line_ids", string="Stock seleccionado"
    )
    reserved_move_ids = fields.One2many(
        "move.change.reserve.line", "wzd_id", string="Reserved Moves"
    )
    quant_ids = fields.One2many(
        "unreserved.available.quant", "wzd_id", string="Unreserved stocks"
    )
    origin = fields.Char("Source Document")
    total_product_uom_qty = fields.Float(
        "Total Demand", digits=dp.get_precision("Product Unit of Measure")
    )
    total_reserved_availability = fields.Float(
        "Total Reserved", digits=dp.get_precision("Product Unit of Measure")
    )
    total_available = fields.Float(
        "Total Available", digits=dp.get_precision("Product Unit of Measure")
    )
    ...
    def autorefresh(self, move_ids):
        ...
        if len(move_ids) == 1:
            ...
            action = self.env.ref(
                "stock_move_reassign_availability.action_stock_move_change_reserve_wzd_form"
            ).read()[0]
        ...
        action["context"] = {
            "reload": True
        }  ## self._context.copy().update(reload=True)
        return action

    def change_reserve_wzd(self):
        to_reserve = self.reserved_move_ids.filtered(
            lambda x: x.reserved_availability == 0.00
        ).sorted(key=lambda l: l.sequence)
        to_unreserve = self.reserved_move_ids.filtered(
            lambda x: x.reserved_availability > 0.00
        ).sorted(key=lambda l: l.sequence)
        for line in to_unreserve:
            line.move_id._do_unreserve()
    ...
    def action_apply_quant(self):
        ...
        if (
            new_location.picking_type_id
            and new_location.picking_type_id != self.move_id.location_id.picking_type_id
        ):
            ...
            if (
                float_compare(
                    quant_id.new_quantity, 0.0, precision_digits=precision_digits
                )
                > 0
            ):
                available_quantity = quant._get_available_quantity(
