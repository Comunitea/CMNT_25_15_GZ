...
class ProductProduct(models.Model):
    ...
    product_putaway_ids = fields.One2many(
        "stock.product.putaway.strategy", "product_product_id", "Ubicaciones de traslado"
    )
    ...
    def open_product_putaway_strat(self):
        ...
        action = self.env.ref(
            "custom_product_putaway.action_stock_fixed_putaway_strat_tree_pp"
        ).read()[0]
        ...
        for product_id in self:
            ...
            if quant:
                ...
                if location_putaway_id.usage == "internal":
                    ...
                    if not line:
                        putaway_ids |= self.env["stock.fixed.putaway.strat"].create(
                            vals
                        )
        for p_id in putaway_ids:
            print(
                "-- {} --> {}".format(
                    p_id.product_id.display_name, p_id.fixed_location_id.name
                )
            )
        xml_id = "custom_product_putaway.action_stock_fixed_putaway_strat_tree_pp"
