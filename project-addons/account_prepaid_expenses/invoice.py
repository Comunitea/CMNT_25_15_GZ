        ...
        for line in move_lines:
            if line[2].get('prepaid_expense_amount_untaxed') and line[2].get('taxes'):
                for tax in line[2]['taxes']:
                    ...
                    if invoice_browse.type in ('out_invoice','in_invoice'):
                        new_dict['tax_amount'] = new_dict['prepaid_expense_amount_untaxed'] * tax.tax_sign
                    else:
                        new_dict['tax_amount'] = new_dict['prepaid_expense_amount_untaxed'] * tax.ref_tax_sign
                    new_dict['debit'] = 0.0
                    new_dict['credit'] = 0.0
    ...
    def compute(self, cr, uid, invoice_id, context=None):
        ...
        for line in inv.invoice_line:
            if line.prepaid_expense_amount:
                for tax in tax_obj.compute_all(cr, uid, line.invoice_line_tax_id, 0.0, line.quantity, inv.address_invoice_id.id, line.product_id, inv.partner_id)['taxes']:
                    tax['price_unit'] = 0.0
                    val={}
        ...
        for t in tax_grouped:
            if tax_grouped[t]['base_amount'] or tax_grouped[t]['tax_amount']:
                tax_grouped[t]['base'] = cur_obj.round(cr, uid, cur, tax_grouped[t]['base'])
