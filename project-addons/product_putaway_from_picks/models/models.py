...
class StockPicking(models.Model):
    ...
    def update_new_product_putaway(self):
        product_ids = self.filtered(lambda x: x.state == 'done').mapped('move_lines').mapped('product_id')
...
class StockPickingBatch(models.Model):
    ...
    @api.multi
    def update_new_product_putaway(self):
        warehouse_ids = self.mapped('picking_ids').mapped('picking_type_id').mapped('warehouse_id')
        product_ids = self.mapped('move_lines').filtered(lambda x: x.state == 'done').mapped('product_id')
        return product_ids._compute_putaway_from_stock(warehouse_ids)
