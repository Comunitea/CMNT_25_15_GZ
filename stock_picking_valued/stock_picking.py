...
class stock_picking(orm.Model):
    ...
    def onchange_partner_in(self, cr, uid, context=None, partner_id=None):
        ...
        if not 'value' in res:
            res['value'] = {}
        if partner_id:
            address_obj = self.pool.get('res.partner').browse(cr, uid, partner_id)
            res['value']['valued_picking'] = address_obj.valued_picking
        return res
...
class stock_move(orm.Model):
    ...
    def _get_subtotal(self, cr, uid, ids, field_name, arg, context):
        ...
        for move in self.browse(cr, uid, ids):
            ...
            if move.sale_line_id:
                ...
                res[move.id]['cost_subtotal'] = cost_price * move.product_qty
                res[move.id]['margin'] = res[move.id]['price_subtotal'] - res[move.id]['cost_subtotal']
                if res[move.id]['price_subtotal'] >0:
                    res[move.id]['percent_margin'] = (res[move.id]['margin']/res[move.id]['price_subtotal'])*100
                else:
                    res[move.id]['percent_margin'] = 0
    ...
    _columns = {
        'price_subtotal': fields.function(_get_subtotal, method=True, string="Subtotal", type="float", digits_compute=dp.get_precision('Sale Price'), readonly=True, multi="order_price",
            store={ 'stock.move': (lambda self, cr, uid, ids, c={}: ids, ['product_qty'], 10)},),
        ...
        'cost_subtotal': fields.function(_get_subtotal, method=True, string="Compra", type="float", digits_compute=dp.get_precision('Purchase Price'), readonly=True, multi="order_price", store=True),
        'margin': fields.function(_get_subtotal, method=True, string="Margen", type="float", digits_compute=dp.get_precision('Sale Price'), readonly=True, multi="order_price", store=True),
        'percent_margin': fields.function(_get_subtotal, method=True, string="% Margen", type="float", digits_compute=dp.get_precision('Sale Price'), readonly=True, multi="order_price"),
    }
