...
class create_facturae(orm.TransientModel):
    ...
    def create_facturae_file(self, cr, uid, ids, context):
        ...
        def _parties_facturae(cr, context):
            ...
            def create_administrative_centres(address, code, dir3):
                texto = ""
                if administrative:
                     texto += "<AdministrativeCentre>"
                     texto += "<CentreCode>" + dir3 + "</CentreCode>"
            ...
            if administrative:
                partner = invoice_partner_address_obj.partner_id
                address = self.pool.get('res.partner').address_get(cr, uid, [partner.id], ["other"])["other"]
                address = address and self.pool.get('res.partner.address').browse(cr, uid, address) or invoice_partner_address_obj
                texto += create_administrative_centres(invoice_partner_address_obj, "01", address.dir3) # Oficina contable
                texto += create_administrative_centres(invoice_partner_address_obj, "02", invoice_partner_address_obj.dir3) # Órgano Gestor
                address = contact_partner_address_obj or invoice_partner_address_obj
                texto += create_administrative_centres(address, "03", address.dir3) # Unidad tramitadora
                texto += "</AdministrativeCentres>"
            ...
            administrative = False
            texto += create_administrative_centres(invoice_partner_address_obj, "00", False)
        ...
        if not invoice_ids or len(invoice_ids) > 1:
            raise orm.except_orm(_('Error !'), _('Only can select one invoice to export'))
